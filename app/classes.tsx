...
import { View,StyleSheet, Text, Image, TouchableOpacity, SafeAreaView, ScrollView, KeyboardAvoidingView, TextInput} from "react-native";
...
export default function Classes() {
    ...
    return (
    <ScrollView scrollEventThrottle={16} bounces={false} style={ styles.container}>
        <SafeAreaView>   
            <Stack.Screen
            options={{
                headerShown: false,
            ...
            <View className="flex flex-row items-center"> 
                ...
                <Text className="ml-4 text-white font-bold text-3xl">CLASES VIRTUALES</Text>
            </View>
                            ...
                            <View className="flex flex-col items-left w-[48%] px-2 py-4"  key={index.toString()}> 
                                <Image
                                    ...
                                    source={{
                                        ...
                                        resizeMode="cover"
                                />
                                <ThemedText type="subtitle" className="w-full">{item.titulo}</ThemedText>
                                <View className="flex flex-row w-full">
                                    <Text className="inline-flex " style={{ color: colorNivel(item.nivel)}}>{ TextNivel(item.nivel) }</Text>
                                    <Text className="inline-flex ml-2 text-white">{item.time}</Text>
                                </View>    
                            </View>    
                        )
                    })
